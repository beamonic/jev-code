...
## Conversation crystallization before planning (#5134)

Use `/skill:deep-interview --crystallize` to turn the current conversation into a versioned requirements specification without repeating settled interview questions. This is a sub-mode of the existing skill, not a new workflow. Ordinary `/skill:deep-interview` behavior remains unchanged; no crystallization state is created before explicit invocation.

The runtime binds extraction to an authenticated, bounded conversation range, revision, and digest. Confirmed requirements retain verbatim user anchors; inferred and disputed material remain distinct. Sufficient evidence produces a draft directly, small gaps receive only blocking questions, and broad ambiguity returns to the ordinary interview. An initial conversation exceeding the 200-message snapshot limit cannot silently discard its prefix: use the ordinary interview rather than promote a tail-only initial Crystal. Later bounded deltas may carry an authenticated prior Crystal forward.

Only the canonical promoter publishes the versioned specification and handoff state. A second crystallization compares later evidence with the prior version: additions preserve unchanged confirmed material, changed requirements reopen affected fields, goal replacement supersedes the old goal, and conflicts prevent a ready result. Version changes invalidate earlier execution approval.

**A ready Crystal is not execution permission.** It starts with `execution_approval: not-approved`. The Phase 5 execution-choice question must carry `workflowGate: { stage: "deep-interview", kind: "execution" }`; only the recognized explicit execution choice can create the user-origin approval record consumed by `approve-execution`. Refinement, research, cancellation, free-form responses, and an untagged question do not authorize implementation. Ralplan planning and its separate exact-plan approval boundary remain intact.

Approval binds the exact published spec or final plan and an authenticated transcript prefix. The workflow's own sequential assistant/tool-result records may follow that prefix; new user evidence, altered prefix bytes, a replaced transcript, or a branch rewind invalidates it. After a newer canonical Crystal invalidates v1 consent, a fresh user choice may approve v2; the old consent remains historical evidence, not reusable permission. Ordinary non-Crystal interview specs and standalone Ralplan plans use explicit artifact-bound approval without requiring crystallization. Retrying a historical final receipt never replaces a newer run or reactivates a cleared or terminal run.

With Ralplan automatic handoff off, naming an execution skill does not bypass the structured approval gate for the current final plan. Headless clients bind consent to the emitted durable gate ID, not a reusable question label. A replacement Ralplan run without incoming handoff identity is independent of historical Deep Interview handoff audits; stale session history cannot attach a prior Crystal to the new plan.
