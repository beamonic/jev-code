...
Skills resolve through `skill://<name>` (and `skill://<name>/<relative path>` for on-disk skills). Bundled skills and skill fragments have no filesystem home, so the skill tool and skill discovery report them as `embedded:gjc/skills/<name>/SKILL.md`; read that identifier verbatim.
